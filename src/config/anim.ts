...
export interface AnimConfig {
  ...
  popMs: number;
  /** Волна сбора бустера до самой дальней клетки, мс (radial span для зонных комбо). */
  boosterWaveMs: number;
  /** Ракета: время пролёта спрайта на всю длину поля (cols/rows), мс. Клетки гаснут по мере прохода. */
  rocketFlyMs: number;
  /** Бомба: длительность вспышки-взрыва, мс (клетки сносятся мгновенно вместе с ней). */
  bombBoomMs: number;
  /** Магнит: фаза «выделения» всех клеток тира (сияние-вибрация), мс; после неё — снос волной сверху вниз. */
  magnetSelectMs: number;
  /** Магнит: шаг волны сноса между соседними РЯДАМИ (сверху вниз), мс. */
  magnetRowMs: number;
