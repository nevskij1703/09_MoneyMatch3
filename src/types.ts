 ...
 * cells[i] === null, тип бустера лежит в special[i].
 *   'bomb'     — рождается из фигуры T/L (пересечение линий 3+ по верт. и гориз.);
 *                при срабатывании сносит область 3×3 вокруг себя.
 *   'rocket-h' — рождается из линии в 4 по ГОРИЗОНТАЛИ; сносит весь свой РЯД.
 *   'rocket-v' — рождается из линии в 4 по ВЕРТИКАЛИ; сносит весь свой СТОЛБЕЦ.
 *   'magnet'   — рождается из линии в 5; собирает ВСЕ плитки одного тира (цель — см.
 *                core/match3.ts: тир соседа по свайпу или случайный ближайший при цепной).
 *   'drone'    — рождается из квадрата 2×2; собирает «плюс» вокруг себя (центр + 4 стороны),
 *                затем «летит» к приоритетной цели (магнит→бомба→ракета→дрон→случайная плитка)
 *                и активирует её. Комбо с другим бустером — см. ui/dom/boardView.ts.
 */
export type SpecialKind = 'bomb' | 'rocket-h' | 'rocket-v' | 'magnet' | 'drone';
