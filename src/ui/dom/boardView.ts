...
//  • 🛸 дрон: на СТАРТЕ взлёта собирает «плюс» вокруг себя, затем (подольше) летит к приоритетной
...
export class BoardView {
  ...
  /** 🛸+💣/🚀 — дрон «уносит» бустер: летит к приоритетной цели и детонирует его в точке приземления. */
  ...
  private async animateStep(step: CascadeStep, timing?: ClearTiming): Promise<void> {
    ...
    let maxPop = 0;
    if (timing && timing.mode === 'radial') for (const idx of step.cleared) maxPop = Math.max(maxPop, popDelay(idx, 0));
    ...
    for (const s of step.spawns) {
      ...
      tile.animate(
        ...
        { duration: 280, delay: gravityDelay, easing: EASE_OUT, fill: 'backwards' },
      );
    }
