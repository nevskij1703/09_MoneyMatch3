...
```jsonc
{
  ...
  "data": {
    ...
    "board": {                    // поле match-3
      "cols": 6, "rows": 6,
      "cells":   [ /* 36 × (Tier|null) — null также под бустером */ ],
      "special": [ /* 36 × ('bomb'|'rocket-h'|'rocket-v'|'magnet'|null) — бустеры-объекты */ ]
    },
...
- **`mergeDefaults`** ([storage.ts](../src/core/storage.ts)) при `load()` собирает `data` поле
  ...
  `'bomb'`/`'rocket-h'`/`'rocket-v'`/`'magnet'`/`null` (старый `'color'` и мусор → `null`).
  Бустер — самостоятельный объект: где он стоит, плитка обнуляется (`cells[i] = null`). При
  загрузке стартовые матчи старого поля тихо схлопываются (`settleInitial` в boardView), затем
  гарантируется ход.
