...
│   └── anim.ts              Скорости анимаций (мс / px·мс): swap/pop/fall/spawn, rocketFlyMs/bombBoomMs/magnetSelectMs/magnetRowMs (свои FX бустеров), reactionMs (гравитация) — читаются boardView на лету; DEV-override (вкладка «Анимации», игнорит устаревшие ключи)
│
...
│   │                        cellsInPlus/droneTargets(includePlus)/pickDroneFlightTarget (дрон: плюс-снос только у ПРЯМОГО (вскрывает и сейф в нём на взлёте); цель полёта — ВНЕ «плюса»; два дрона не берут одну цель — claimed),
...
1. [boardView.ts](../src/ui/dom/boardView.ts) `pointerdown` запоминает клетку, `pointermove` за
   ...
   💎/⚡ свапаются как фишки (обмен прилипает по матчу; свайп на бустер активирует и собирает их); 🎁 сейф НЕПОДВИЖЕН — свайп с его участием просто игнорируется (ни обмена, ни энергии, ни анимации — сейф даже не дёргается), вскрывается только прямым попаданием бустера. Валидный ход → `onSpendEnergy()` СРАЗУ (в момент свайпа).
2. Каскад-петля: `resolveStep` (`findMatches` → `applyClear(..., byMatch=true)`: 💎/⚡ ловятся
   соседством с НАТУРАЛЬНЫМ матчем (при активации бустера, `byMatch=false`, — только ПРЯМЫМ попаданием);
   🎁 сейф — ВСЕГДА только прямым попаданием бустера → обнуление → `applyGravityAndRefill` со спавном
   💎/⚡/🎁) пока есть матчи. `step.groups` = число
...
4. Анимация шага (`animateStep` — НЕ ждёт сам, возвращает `{settleMs, colSettle}`): pop схлопнутых —
   фишка РЕЗКО сжимается в ноль + радиальная световая волна (`matchWave`); для бустеров pop ПОСТЕПЕННО
   (radial; бомба — instant + вспышка); полёт собранных 💎/⚡, открытие 🎁 (растворение+вспышка+баунс
   награды), падение уцелевших + досыпка РЕАКЦИЯ-каскадом снизу-вверх (anim.reactionMs; досыпка по одному).
   **Гравитация ждёт ВСЕ эффекты** (`gravityDelay` = max по ВСЕМ `delays`, не только `step.cleared`) —
   иначе полёт дрона к 🎁 СЕЙФУ (цель уходит из `cleared` в спавн) стартовал бы гравитацию до приземления
   и поле «съезжало» под дроном. **Каскад без простоя:** `runNaturalCascade` подглядывает следующий матч
   (`findMatches`, pure) и ждёт оседания только ЕГО столбцов (`colSettle`), а не всего поля. Свайп бустера
   на плитку с матчем (`boosterSwapWithMatch`): матч + активация бустера ОДНИМ шагом (`detonateBlasts` со
   `spawns`/`keep`) — бустер срабатывает СРАЗУ; новые бустеры из матча иммунны (выживают, не детонируют).
5. В конце хода `onMoveEnd()` → `economy.commitMove` (Баланс растёт ОДИН раз), деньги улетают в
   **карту** (`flyMoneyToBalance` → `card.refresh()` + bump). Бустеры активируются МГНОВЕННО (после
   ...
   магнит-спавн / 3 дрона / дрон-уносит-бустер. Энергия списана на свайпе (шаг 1). Анти-дедлок. Сейв.
