...
4. Анимация шага (`animateStep` — НЕ ждёт сам, возвращает `{settleMs, colSettle}`): pop схлопнутых —
   ...
   **Каскад без простоя:** `runNaturalCascade` подглядывает следующий матч (`findMatches`, pure) и ждёт
   только оседания ЕГО столбцов (`colSettle`), а не всего поля. Свайп бустера на плитку с матчем: сперва
   схлоп матча (родить бустер), затем активация бустера — новые бустеры иммунны.
