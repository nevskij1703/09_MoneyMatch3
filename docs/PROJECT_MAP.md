...
1. [boardView.ts](../src/ui/dom/boardView.ts) `pointerdown` запоминает клетку, `pointermove` за
   ...
   💎/⚡ свапаются как фишки (обмен прилипает по матчу; свайп на бустер активирует и собирает их); 🎁 сейф НЕПОДВИЖЕН — свайп отклоняется (`rejectSwap`), вскрывается только прямым попаданием бустера. Валидный ход → `onSpendEnergy()` СРАЗУ (в момент свайпа).
